...
create table if not exists public.product_customer_fields (
  id uuid primary key default gen_random_uuid(),
  product_id uuid not null references public.products(id) on delete cascade,
  label text not null check (char_length(btrim(label)) between 1 and 80),
  placeholder text check (placeholder is null or char_length(placeholder) <= 150),
  field_type text not null default 'TEXT'
    check (field_type in ('TEXT', 'EMAIL', 'NUMBER', 'TEXTAREA')),
...
create index if not exists product_customer_fields_product_sort_idx
  on public.product_customer_fields(product_id, sort_order, created_at);

alter table public.product_customer_fields enable row level security;

revoke all on table public.product_customer_fields from anon, authenticated;
grant select on table public.product_customer_fields to anon, authenticated;

drop policy if exists "Public reads active product customer fields"
  on public.product_customer_fields;

create policy "Public reads active product customer fields"
  on public.product_customer_fields
  for select
  ...
  using (
    exists (
      ...
      where products.id = product_customer_fields.product_id
...
alter table public.order_items
  add column if not exists customer_information jsonb not null default '[]'::jsonb;

comment on table public.product_customer_fields is
  'Admin-configured customer information fields shown on an individual product.';

comment on column public.order_items.customer_information is
  'Validated customer information captured for this order item, with field labels snapshotted at purchase time.';
...
alter table public.product_options
  add column if not exists minimum_quantity integer check (minimum_quantity is null or minimum_quantity >= 1),
  add column if not exists maximum_quantity integer check (maximum_quantity is null or maximum_quantity >= 1);
