...
export default function AdminHomepageSlider({
  ...
  return (
    <div className="mt-7">
      <form
        action={saveSliderSettings}
        className="flex flex-wrap items-center gap-5 rounded-2xl border border-slate-200 bg-slate-50 p-4"
      >
        ...
        <label className="flex items-center gap-2 text-sm font-bold text-slate-600">
          ...
          <input
            ...
            min="2"
            max="30"
            defaultValue={Math.round(settings.autoplay_ms / 1000)}
            className="w-20 rounded-lg border border-slate-200 bg-white px-3 py-2"
          />
          seconds
        </label>

        <button className="ml-auto rounded-xl bg-slate-900 px-5 py-2.5 font-bold text-white">
      ...
      <div className="mt-5 grid min-w-0 gap-5">
        <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={() => setDraft(emptySlide)}
              className="rounded-lg bg-cyan-500 px-3 py-2 text-sm font-black text-slate-950"
            >
              + Add slide
            </button>
          </div>
          ...
          <div className="mt-4 grid gap-3">
              ...
              >
                ...
                >
                  ...
                  <img
                    ...
                    alt=""
                    className="aspect-[1920/700] w-28 rounded-lg bg-slate-100 object-fill"
                  />
            ...
            {slides.length === 0 && (
              <p className="rounded-xl bg-slate-50 p-5 text-center text-sm text-slate-500">
                No slides yet. Select Add slide.
              </p>
            )}
          </div>
        </section>
        ...
        <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Desktop image and Buy Now button only. Images automatically fill
            the fixed slider frame.
          </p>
          ...
          <form action={saveSlide} className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Buy Now link" wide>
              <input
                name="button_url"
                value={draft.button_url}
                readOnly={Boolean(draft.product_id)}
                onChange={(event) =>
                  setDraft({ ...draft, button_url: event.target.value })
                }
                className={`${inputClass} read-only:bg-slate-100`}
                placeholder="/products"
              />
            </Field>

            <Field label="Start date (India)">
              <input
                type="datetime-local"
                name="starts_at"
                defaultValue={localDate(draft.starts_at)}
                key={`start-${draft.id}`}
                className={inputClass}
              />
            </Field>

            <Field label="End date (India)">
              <input
                type="datetime-local"
                name="ends_at"
                defaultValue={localDate(draft.ends_at)}
                key={`end-${draft.id}`}
                className={inputClass}
              />
            </Field>
            ...
            <div className="sm:col-span-2">
              <p className="mb-2 text-sm font-bold text-slate-700">
                Fixed desktop frame preview
              </p>
              <div className="relative aspect-[1920/700] w-full overflow-hidden rounded-2xl bg-slate-900">
            ...
            <button className="sm:col-span-2 justify-self-end rounded-xl bg-cyan-500 px-6 py-3 font-black text-slate-950">
              {draft.id ? "Save changes" : "Create slide"}
            </button>
          </form>
        </section>
      </div>
...
const inputClass =
  "mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-100";
