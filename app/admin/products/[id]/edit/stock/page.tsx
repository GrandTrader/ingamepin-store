...
export default async function ProductStockPage({ params, searchParams }: {
  ...
  const [productResult, optionsResult, codesResult, digisellerResult] = await Promise.all([
    admin.from("products").select("id, name, slug, stock_quantity").eq("id", id).maybeSingle(),
    admin.from("product_options").select("id, option_name, denomination, denomination_currency, stock_quantity, digiseller_product_id").eq("product_id", id).eq("is_active", true).eq("is_custom_value", false).order("sort_order"),
    ...
    listDigiSellerProducts().then((products) => ({ products })).catch((cause: unknown) => ({ products: [], error: cause instanceof Error ? cause.message : "Unable to load DigiSeller products." })),
  ]);
  ...
  return <div className="min-h-screen bg-white text-slate-900"><div className="mx-auto flex min-h-screen max-w-[1500px] flex-col lg:flex-row"><AdminSidebar /><main className="min-w-0 flex-1 p-5 sm:p-8">
    ...
    <DigiSellerMapping productId={id} options={(optionsResult.data ?? []).map((option) => ({ id: option.id, name: option.option_name, digisellerProductId: option.digiseller_product_id === null ? null : Number(option.digiseller_product_id) }))} products={digisellerResult.products} loadError={"error" in digisellerResult ? digisellerResult.error : undefined} />
  </main></div></div>;
}
