...
export async function saveProductOptions(formData: FormData) {
  ...
  const product = await admin.from("products").select("category_id").eq("id", productId).maybeSingle();
  ...
  for (const [index, option] of options.entries()) {
    const values = { category_id: product.data.category_id, option_type: "CURRENCY", option_name: option.name.trim(), denomination: option.denomination, denomination_currency: option.currency, selling_price: option.sellingPrice, sort_order: index, is_active: option.isActive, is_custom_value: false };
    const result = option.id
      ? await admin.from("product_options").update(values).eq("id", option.id).eq("product_id", productId)
      : await admin.from("product_options").insert({ ...values, product_id: productId, stock_quantity: 0 });
