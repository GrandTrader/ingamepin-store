...
type Option = { id: string; name: string; denomination: number; currency: string; sellingPrice: number; isActive: boolean };
...
export default function ProductOptionsEditor({ initialOptions, productName }: { initialOptions: Option[]; productName: string }) {
  ...
  function add() { setOptions((current) => [...current, { id: "", name: "", denomination: 1, currency: "INR", sellingPrice: 0, isActive: true }]); setSelected(options.length); }
  ...
  return <><input type="hidden" name="options" value={JSON.stringify(options)} />
    <div className="grid gap-5 xl:grid-cols-[190px_1fr_280px]">
      ...
      <section className="min-w-0"><div className="mb-5"><h2 className="text-xl font-black">Denomination options</h2><p className="mt-1 text-sm text-slate-500">Edit the customer-visible options and selling prices.</p></div><div className="overflow-x-auto"><div className="min-w-[650px]">
        <div className="grid grid-cols-[34px_1.6fr_110px_100px_120px_80px_42px] gap-3 border-b border-slate-200 px-2 pb-2 text-xs font-bold uppercase tracking-wide text-slate-500"><span></span><span>Option name</span><span>Value</span><span>Currency</span><span>Price USD</span><span>Default</span><span></span></div>
        {options.map((option, index) => <div key={option.id || `row-${index}`} draggable onDragStart={() => setDragging(index)} onDragOver={(event) => event.preventDefault()} onDrop={() => drop(index)} className={`grid grid-cols-[34px_1.6fr_110px_100px_120px_80px_42px] items-center gap-3 border-b border-slate-200 px-2 py-3 ${selected === index ? "bg-blue-50" : ""}`} onClick={() => setSelected(index)}>
      ...
      <aside className="h-fit rounded-xl border border-slate-200 bg-slate-50 p-4 shadow-sm"><p className="text-sm font-black">Preview payment form</p><div className="mt-4 rounded-lg border border-slate-200 bg-white p-4"><p className="text-sm font-bold text-slate-700">{productName}</p><div className="mt-4 grid gap-3">{options.filter((option) => option.isActive).map((option, index) => <label key={option.id || `preview-${index}`} className="flex cursor-pointer items-center gap-2 text-sm"><input type="radio" name="preview_option" checked={selected === options.indexOf(option)} onChange={() => setSelected(options.indexOf(option))} /><span>{option.name || `${option.denomination} ${option.currency}`}</span><span className="ml-auto font-bold">${option.sellingPrice.toFixed(2)}</span></label>)}</div><button type="button" className="mt-5 w-full rounded-lg bg-slate-300 px-4 py-3 font-black text-white">BUY</button></div></aside>
    </div>
  </>;
}
