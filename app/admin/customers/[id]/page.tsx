          ...
          <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 overflow-x-auto">
              <table className="min-w-[850px] w-full text-left text-sm">
                ...
                <tbody>
                  {transactions.map((item) => <tr key={item.id} className="border-b border-slate-100"><td className="p-3">{date(item.created_at)}</td><td className="p-3 font-bold">{item.transaction_type}</td><td className="p-3 font-black">{money(Number(item.amount), wallet?.currency ?? "USD")}</td><td className="p-3">{money(Number(item.balance_before), wallet?.currency ?? "USD")}</td><td className="p-3">{money(Number(item.balance_after), wallet?.currency ?? "USD")}</td><td className="p-3">{item.description}</td><td className="p-3 text-xs">{item.reference_id ?? "—"}</td><td className="p-3"><Link href={`/admin/customers/${id}/transactions/${item.id}`} prefetch={false} className="inline-flex whitespace-nowrap rounded-lg bg-blue-600 px-3 py-2 text-xs font-bold text-white hover:bg-blue-700">View receipt</Link></td></tr>)}
                  {transactions.length === 0 && <tr><td colSpan={8} className="p-8 text-center text-slate-500">No wallet activity yet.</td></tr>}
                </tbody>
              </table>
            </div>
          </section>
          ...
          <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 overflow-x-auto">
              <table className="min-w-[700px] w-full text-left text-sm">
                ...
                <tbody>
                  {orders.map((order) => <tr key={order.id} className="border-b border-slate-100"><td className="p-3 font-black"><Link href={`/admin/orders/${order.id}/receipt`} className="text-cyan-600 underline underline-offset-2 hover:text-cyan-500">{order.order_number}</Link></td><td className="p-3">{date(order.created_at)}</td><td className="p-3 font-black">{money(Number(order.total), order.currency)}</td><td className="p-3"><span className={`rounded-full px-3 py-1 text-xs font-black ${statusClass(order.status)}`}>{order.status.replaceAll("_", " ")}</span></td><td className="p-3"><Link href={`/admin/orders/${order.id}/receipt`} className="font-bold text-cyan-600">View</Link></td></tr>)}
