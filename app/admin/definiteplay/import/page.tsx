...
import {supplierProductVariant} from "@/lib/definiteplay-import";
import SupplierImportForm from "./SupplierImportForm";

export const dynamic="force-dynamic";
export default async function SupplierImportPage({searchParams}:{searchParams:Promise<{q?:string;category?:string;region?:string;sku?:string;single?:string;variant?:string}>}) {
  ...
  const back="/admin/definiteplay?"+new URLSearchParams({q,category,region,variant});
  const categories=await session.from("categories").select("id,name").eq("is_active",true).order("name");
  let data=null,error="",groupLabel="";
  try {
    data=sku?await getDefinitePlayItems([sku]):await getDefinitePlayCatalogue(q,0,51,{category,region,variant});
    if(sku && value("single")!=="1" && !data.stale && data.items.length===1) {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900"><div className="mx-auto flex min-h-screen max-w-[1500px] flex-col lg:flex-row"><AdminSidebar/><main className="min-w-0 flex-1 p-5 sm:p-8">
    ...
    {error?<div className="mt-6 rounded-xl bg-amber-50 p-4 text-amber-900"><p role="alert">{error}</p>
      {!sku&&data&&new Set(data.items.map(supplierProductVariant)).size>1&&<div className="mt-3 flex flex-wrap gap-4">
      ...
      {sku&&data&&data.total>50&&<Link href={"/admin/definiteplay/import?"+new URLSearchParams({sku,single:"1",q,category,region,variant})} className="mt-3 inline-block font-bold text-blue-600">Import only the chosen denomination</Link>}
